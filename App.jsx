...
function modelReducer(state, action) {
  switch (action.type) {
    ...
    case "UPDATE_NODE": {
      const nodes = state.nodes.map((n) => (n.id === action.payload.id ? { ...n, ...action.payload } : n));
      return { ...state, nodes };
    }
    case "REMOVE_NODE": {
      const nodes = state.nodes.filter((n) => n.id !== action.payload.id);
      // Cascade: drop any element that referenced this node.
      const elements = state.elements.filter((el) => {
        const refs = el.kind === "SLAB" ? el.boundaryNodeIds : el.nodeIds;
        return !refs.includes(action.payload.id);
      });
      return { ...state, nodes, elements };
    }
    case "ADD_ELEMENT":
      return { ...state, elements: [...state.elements, action.payload] };
    case "REMOVE_ELEMENT": {
      const elements = state.elements.filter((el) => el.id !== action.payload.id);
      return { ...state, elements };
    }
    case "SET_SLAB_LOAD": {
      const others = state.slabLoads.filter((l) => l.slabId !== action.payload.slabId);
      return { ...state, slabLoads: [...others, action.payload] };
    }
