"""
Mobile-ETABS backend — FastAPI service.

Endpoints:
  POST /api/analyze   -> full pipeline: validate -> tributary load transfer -> frame analysis
                          -> IS 456:2000 beam & column design -> combined result payload.
  GET  /api/health     -> liveness check.

Analysis: classical 3D direct-stiffness method (Euler-Bernoulli, 6 DOF/node, 12 DOF/element).
No external FE dependency (OpenSeesPy/anastruct) is required to run this file — numpy is
the only hard dependency — but the element assembly is written so OpenSeesPy could be
swapped in behind `solve_frame()` without touching the rest of the pipeline.

Units convention (matches src/types.ts):
  length: m (mm only for section b/D/h and rebar areas)
  force: kN
  stress: N/mm^2 (MPa)
Internally the solver works in a consistent SI set (N, m) and converts at the boundary.
"""
